# =========================================================
# MÓDULO: LÓGICA DE GRAFOS, RUTAS Y ADMINISTRACIÓN
# =========================================================
# Este archivo gestiona la red de nodos, el cálculo de rutas 
# mediante Dijkstra y el control de acceso administrativo.
...
# Definición del Blueprint para organizar las rutas del sistema de grafos
grafos_bp = Blueprint('grafos', __name__)

# ---------------------------------------------------------
# CONSTRUCCIÓN DE LA ESTRUCTURA DEL MAPA
# ---------------------------------------------------------
def obtener_grafo():
    """
    Crea una instancia de un Grafo No Dirigido.
    Define los puntos (nodos) y los caminos (aristas) del sistema.
    """
    G = nx.Graph()
    
    # Generación de la lista maestra de nodos (Especiales + 1 al 320)
    nodos_totales = [
        "Esq_Sup_Izq", "Esq_Sup_Der", "Esq_Inf_Izq", "Esq_Inf_Der", 
        "Nucleo_Laberinto"] + [str(i) for i in range(1, 321)]
    
    # Registro formal de todos los nodos en la estructura de NetworkX
    G.add_nodes_from(nodos_totales)

    # Definición de la topología: Conexiones físicas entre puntos
    conexiones = [
        ...
        ("311", "310"), ("310", "309"), ("310", "300"), ("309", "308"), ("300", "299"), ("300", "301"),
    ...
    # Inserción de aristas en el grafo con peso predeterminado de 1
...
def verificar_grafo(G):
    """ Inspecciona la integridad del grafo buscando nodos aislados o cortes en la red """
...
def limpiar_bloqueos():
    """ Redirige al usuario al índice para refrescar el estado del mapa """
    return redirect(url_for('grafos.index'))

# ---------------------------------------------------------
# GESTIÓN DE PERSISTENCIA (CRUD: Crear Favoritos)
# ---------------------------------------------------------
@grafos_bp.route('/guardar_favorita', methods=['POST'])
def guardar_favorita():
    """ Guarda la configuración actual de una ruta en la base de datos del usuario """
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('main.home'))

    # Extracción de parámetros enviados desde el mapa interactivo
    ...
    try:
        # Creación del objeto de modelo para la base de datos
...
# ---------------------------------------------------------
# CONTROL ADMINISTRATIVO (SEGURIDAD Y ROLES)
# ---------------------------------------------------------
@grafos_bp.route('/admin/usuarios')
def panel_admin():
    """ Valida privilegios y muestra la lista de usuarios registrados """
...
def cambiar_rango(id, nuevo_rol):
    """ 
    Restringe la alteración de rangos. 
    Solo el Administrador Principal (ID 1) puede ejecutar esta acción.
    """
    ...
    if usuario:
        # Blindaje del Administrador Central
        if usuario.id == 1:
...
def eliminar_usuario(id):
    """ Proceso de baja de usuarios en el sistema (Exclusivo para ID 1) """
    if session.get('user_id') != 1:
        abort(403)
...
def actualizar_bloqueos_globales():
    """ 
    Modifica el estado global del mapa para todos los usuarios.
    Guarda la lista de nodos restringidos en la tabla 'estado_mapa'.
    """
    if session.get('user_id') != 1:
        abort(403)
    ...
    if estado:
        ...
        # Creación del registro inicial si no existe
...
# ---------------------------------------------------------
# PROCESAMIENTO PRINCIPAL DE RUTAS (DIJKSTRA)
# ---------------------------------------------------------
@grafos_bp.route('/', methods=['GET', 'POST'])
def index():
    """ Controlador principal del mapa interactivo """
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('main.home'))

    # Inicialización del grafo base
    G = obtener_grafo()
    
    # Aplicación de restricciones de entorno (Bloqueos Globales del Admin)
    estado = EstadoMapa.query.get(1)
    ...
    for nodo in bloqueos_admin:
        if nodo in G:
            for vecino in list(G.neighbors(nodo)):
                G[nodo][vecino]['weight'] = 9999 # Deshabilitación lógica de la arista
    ...
    if request.method == 'POST':
        ...
        # Aplicación de bloqueos temporales definidos por el usuario actual
        ...
        # Ejecución del algoritmo de búsqueda de camino más corto
